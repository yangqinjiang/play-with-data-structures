...
//优化
//接口:向二分搜索树中添加新的元素e
func (this *BST)Add(key string,value int)  {
	this.root = this._Add(this.root,key,value)
}
//优化
// 向以node为根的二分搜索树中插入元素e，递归算法
// 返回插入新节点后二分搜索树的根
func (this *BST)_Add(node *Node,key string,value int) *Node  {
	if nil == node{
		this.size ++
...
func (this *BST)getNode(node *Node,key string) *Node {
	if node == nil{
		return nil
	}
	if key == node.Key{//找到
...
//接口:看二分搜树中是否包含元素e
func (this *BST)Contains(key string) bool {
	return this.getNode(this.root,key) != nil
}
//接口:获取k-v,不存在则返回 0
func (this *BST)Get(key string) int  {
...
//接口:更新k-v
func (this *BST)Set(key string,newValue int)  {
	node := this.getNode(this.root,key)
	if node == nil{
...
//使用后继节点的方式,删除任意元素
//接口:从二分搜索树中删除元素为e的节点
func (this *BST)Remove(key string) int {
	node := this.getNode(this.root,key)
	if node != nil{
...
// 返回删除节点后,新的二分搜索树的根
func (this *BST)_remove(node *Node,key string) *Node  {
	if nil == node{
		return nil
	}
