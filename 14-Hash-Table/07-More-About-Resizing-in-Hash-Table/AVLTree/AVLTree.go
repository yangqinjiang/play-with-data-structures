...
func (this *AVLTree)_Add(node *Node,key string,value int) *Node  {
	...
	if balanceFactor > 1 && this.getBalanceFactor(node.Left) < 0{
		// 先左转node.Left,后右转node
		node.Left = this.leftRotate(node.Left)//LR
		return this.rightRotate(node)
	}
	if balanceFactor < -1 && this.getBalanceFactor(node.Right) > 0{
		// 先右转node.Right,后左转node
		node.Right = this.rightRotate(node.Right)//RL
...
func (this *AVLTree)_remove(node *Node,key string) *Node  {
	...
	if key < node.Key{ //从左子树查询,删除
		...
		if nil == node.Left{
			...
			//return rightNode
			retNode = rightNode
		}else if nil == node.Right {//待删除节点右子树为空的情况
			leftNode := node.Left
	...
	//更新节点的height
	retNode.height = 1 + this.max(this.getHeight(retNode.Left),this.getHeight(retNode.Right))
	...
	if balanceFactor > 1 && this.getBalanceFactor(retNode.Left) >= 0{
		return this.rightRotate(retNode) //RR
	}
	if balanceFactor < -1 && this.getBalanceFactor(retNode.Right) <= 0{
		return this.leftRotate(retNode) // LL
	}
	if balanceFactor > 1 && this.getBalanceFactor(retNode.Left) < 0{
		// 先左转node.Left,后右转node
		retNode.Left = this.leftRotate(retNode.Left)//LR
		return this.rightRotate(retNode)
	}
	if balanceFactor < -1 && this.getBalanceFactor(retNode.Right) > 0{
		// 先右转node.Right,后左转node
		retNode.Right = this.rightRotate(retNode.Right)//RL
